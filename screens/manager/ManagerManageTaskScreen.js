...
import ManagerCreateTaskScreen from './ManagerCreateTaskScreen';
...
export default function ManagerManageTaskScreen() {
  return (
    <Stack.Navigator 
    ...
    screenOptions={{
      ...
      <Stack.Screen 
        name="CreateTask" 
        component={ManagerCreateTaskScreen}
        options={
          (props) => ({
            title: "Create Task",
          })
...
function ManageTask(props) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await fetch('http://' + ip + '/api/task/read-by-group.php?group_id=' + info.group_id);
      const result = await response.json();
      let arr = new Array;
  ...
  return (
    ...
    >
      {list[0] != "This group has not been assigned any task" ? (
        <>
