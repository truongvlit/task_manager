...
import { Text, StyleSheet, View, ToastAndroid, Button, KeyboardAvoidingView, AsyncStorage, Vibration } from 'react-native';
...
export default function UserCreateTaskScreen(props) {
  return(
...
const CreateUserForm = (props) => {
  ...
  const createTask = async() => {
    if (validate()) {
      ...
      try {
        ...
        const response = await fetch('http://' + ip + '/api/task/request-task.php', {
          method: 'POST',
          body: JSON.stringify({
  ...
  const sendPushNotification = async() => {
    const message = {
      ...
      sound: 'default',
      title: 'Task Manager',
      body: 'A user has just requested a new task!',
      data: { data: 'goes here' },
    };
  ...
  return(
    <View>
      <View style={styles.form}>
        <Row title="Task content: " value={content} placeholder="Enter task content" onChange={setContent}/>
        ...
        <View style={{flexDirection: "row", justifyContent: "space-between", marginTop: 185}}>
        <Button 
          title="                Create                " 
          onPress={() => {createTask()}}
        />
